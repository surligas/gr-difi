...
BOOST_AUTO_TEST_CASE(t_simd_endian_swap_16)
{
  ...
  for (size_t n : test_sizes) {
    ...
    if constexpr (std::endian::native == std::endian::little) {
      for (size_t i = 0; i < n; ++i) {
        uint16_t expected = static_cast<uint16_t>(((original[i] & 0xff) << 8) | ((original[i] >> 8) & 0xff));
        BOOST_CHECK_EQUAL(data[i], expected);
      }
    }

    // Swapping again should restore original data
    difi::swap_endian_16(data.data(), n);
    for (size_t i = 0; i < n; ++i) {
...
BOOST_AUTO_TEST_CASE(t_context_standard_108_parse_and_pack)
{
  ...
  difi_context ctx = difi::make_default_context(
      samp_rate, 16, stream_id, full, frac, difi::STANDARD_CONTEXT_SIZE);

  BOOST_CHECK_EQUAL(ctx.packet_size, difi::STANDARD_CONTEXT_SIZE);
  BOOST_CHECK_EQUAL(ctx.raw.size(), difi::STANDARD_CONTEXT_SIZE);

  // Parse the raw bytes
  difi_context parsed = difi::parse_context(ctx.raw.data(), ctx.raw.size());
  ...
  BOOST_CHECK_EQUAL(parsed.payload_format, SIXTEEN_BIT_SIGNED_CART_LINK_EFF);
}
...
BOOST_AUTO_TEST_CASE(t_context_alt_72_parse_and_pack)
{
  ...
  difi_context ctx = difi::make_default_context(
      samp_rate, 8, stream_id, 0, 0, difi::ALT_CONTEXT_SIZE);
  ...
  BOOST_CHECK_CLOSE(parsed.bandwidth, samp_rate * 0.8, 0.01);
  BOOST_CHECK_CLOSE(parsed.sample_rate, samp_rate, 0.01);
  BOOST_CHECK_EQUAL(parsed.payload_format, EIGHT_BIT_SIGNED_CART_LINK_EFF);
}
...
BOOST_AUTO_TEST_CASE(t_sample_packing_and_unpacking_16bit)
{
  ...
  for (size_t i = 0; i < N; ++i) {
    samples_in[i] = gr_complex(static_cast<float>(i * 10), static_cast<float>(-static_cast<int>(i) * 10));
  }
  ...
  size_t unpacked = difi::unpack_samples(payload.data(), payload.size(), samples_out.data(), N, 16);
...
BOOST_AUTO_TEST_CASE(t_sample_packing_and_unpacking_8bit)
{
  ...
  std::vector<gr_complex> samples_in(N);
  for (size_t i = 0; i < N; ++i) {
    samples_in[i] = gr_complex(static_cast<float>(static_cast<int>(i) - 32), static_cast<float>(32 - static_cast<int>(i)));
  }
  ...
  std::vector<gr_complex> samples_out(N);
  size_t unpacked = difi::unpack_samples(payload.data(), payload.size(), samples_out.data(), N, 8);
...
BOOST_AUTO_TEST_CASE(t_pmt_conversions)
{
  ...
  difi_context ctx = difi::make_default_context(
      1000000.0, 16, 7, 50, 123456ULL, difi::STANDARD_CONTEXT_SIZE);

  pmt::pmt_t dict = difi::context_to_pmt(h, ctx);
  BOOST_CHECK(pmt::is_dict(dict));

  difi_context parsed_ctx = difi::pmt_to_context(dict);
  ...
  BOOST_CHECK_EQUAL(pmt::to_uint64(pmt::dict_ref(pkt_dict, pmt::intern("pck_n"), pmt::get_PMT_NIL())), 3);
  BOOST_CHECK_EQUAL(pmt::to_uint64(pmt::dict_ref(pkt_dict, pmt::intern("data_len"), pmt::get_PMT_NIL())), 1400);
}
