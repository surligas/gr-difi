...
difi_header difi::parse_header(const void *buf, size_t len) {
  if (len < HEADER_SIZE) {
...
void difi::pack_header(void *buf, const difi_header &header) {
  ...
  if (w0 == 0) {
    ...
    if (static_part == 0) {
      static_part = (header.type == packet_type::context)
                        ? STANDARD_CONTEXT_STATIC_BITS
                        : STANDARD_DATA_STATIC_BITS;
    }
...
difi_context difi::parse_context(const void *buf, size_t len) {
  if (len < ALT_CONTEXT_SIZE) {
    throw std::runtime_error("Context packet length " + std::to_string(len) +
                             " is smaller than minimum context size (72 bytes)");
  }
  ...
  auto unpack_u32 = [](const uint8_t *ptr) {
    uint32_t val;
    std::memcpy(&val, ptr, 4);
    return ntohl(val);
  };
  auto unpack_u64 = [](const uint8_t *ptr) {
    uint64_t val;
  ...
  if (len == ALT_CONTEXT_SIZE) {
    ...
    ctx.state_and_event_indicators =
        unpack_u32(&in[CONTEXT_PACKET_ALT_OFFSETS[idx++]]);
    ...
    ctx.ref_level = vita_fixed_to_float(static_cast<int16_t>(
        0xffffU & unpack_u32(&in[CONTEXT_PACKET_OFFSETS[idx++]])));
    ...
    ctx.timestamp_adjustment =
        static_cast<int64_t>(unpack_u64(&in[CONTEXT_PACKET_OFFSETS[idx++]]));
    ctx.timestamp_calibration_time =
        unpack_u32(&in[CONTEXT_PACKET_OFFSETS[idx++]]);
    ctx.state_and_event_indicators =
        unpack_u32(&in[CONTEXT_PACKET_OFFSETS[idx++]]);
...
std::vector<uint8_t> difi::pack_context(const difi_context &ctx,
                                        uint8_t pkt_n,
                                        uint32_t stream_id,
                                        size_t packet_size) {
  std::vector<uint8_t> out(packet_size, 0);

  auto pack_u32 = [](uint8_t *ptr, uint32_t val) {
    uint32_t net = htonl(val);
    std::memcpy(ptr, &net, 4);
  };
  auto pack_u64 = [](uint8_t *ptr, uint64_t val) {
    uint64_t net = htobe64(val);
    std::memcpy(ptr, &net, 8);
  };

  uint32_t static_bits = (packet_size == ALT_CONTEXT_SIZE)
                             ? ALT_CONTEXT_STATIC_BITS
                             : STANDARD_CONTEXT_STATIC_BITS;
  uint32_t header = static_bits |
  ...
  if (packet_size == ALT_CONTEXT_SIZE) {
    ...
    pack_u64(&out[CONTEXT_PACKET_ALT_OFFSETS[idx++]], 0);
    pack_u64(&out[CONTEXT_PACKET_ALT_OFFSETS[idx++]], 0);
    pack_u64(&out[CONTEXT_PACKET_ALT_OFFSETS[idx++]], 0);
    pack_u64(&out[CONTEXT_PACKET_ALT_OFFSETS[idx++]],
    ...
    pack_u32(&out[CONTEXT_PACKET_OFFSETS[idx++]],
             static_cast<uint32_t>(static_cast<uint16_t>(
                 float_to_vita_fixed(ctx.ref_level))));
    uint32_t gain_word =
        (static_cast<uint32_t>(static_cast<uint16_t>(
             float_to_vita_fixed(ctx.if_gain)))
         << 16) |
        (static_cast<uint32_t>(static_cast<uint16_t>(
            float_to_vita_fixed(ctx.rf_gain))));
    ...
    pack_u32(&out[CONTEXT_PACKET_OFFSETS[idx++]],
             ctx.timestamp_calibration_time != 0
                 ? ctx.timestamp_calibration_time
                 : ctx.full);
    pack_u32(&out[CONTEXT_PACKET_OFFSETS[idx++]],
             ctx.state_and_event_indicators != 0
...
difi_context difi::make_default_context(double samp_rate,
                                        int depth,
                                        uint32_t stream_id,
                                        uint32_t full,
                                        uint64_t frac,
  ...
  difi_context ctx;
  uint64_t base_oui =
      (packet_size == ALT_CONTEXT_SIZE) ? ALT_OUI : STANDARD_OUI;
  ctx.class_id = (base_oui << 32) ^ 1;
  ctx.full = full;
  ctx.frac = frac;
  ctx.cif = (packet_size == ALT_CONTEXT_SIZE) ? 966885376U : 0xfbb98000U;
  ctx.ref_point = 0x64;
  ...
  auto packed = pack_context(ctx, 0, stream_id, packet_size);
...
difi::advance_timestamp(uint32_t full,
                        uint64_t frac,
                        size_t num_samples,
                        double samp_rate) {
...
template <typename T>
size_t difi::unpack_samples(const void *payload,
...
pmt::pmt_t difi::context_to_pmt(const difi_header &header,
                                const difi_context &ctx) {
  pmt::pmt_t dict = pmt::make_dict();
  dict = pmt::dict_add(dict, pmt::intern("header"),
                       pmt::from_long(header.raw_header));
  dict = pmt::dict_add(dict, pmt::intern("stream_num"),
                       pmt::from_uint64(header.stream_id));
  dict = pmt::dict_add(dict, pmt::intern("class_id"),
                       pmt::from_long(ctx.class_id));
  ...
  dict = pmt::dict_add(dict, pmt::intern("bandwidth"),
                       pmt::from_double(ctx.bandwidth));
  dict = pmt::dict_add(dict, pmt::intern("if_reference_frequency"),
                       pmt::from_double(ctx.if_ref_freq));
  dict = pmt::dict_add(dict, pmt::intern("rf_reference_frequency"),
                       pmt::from_double(ctx.rf_ref_freq));
  dict = pmt::dict_add(dict, pmt::intern("if_band_offset"),
                       pmt::from_double(ctx.if_band_offset));
  dict = pmt::dict_add(dict, pmt::intern("samp_rate"),
                       pmt::from_double(ctx.sample_rate));
  dict = pmt::dict_add(dict, pmt::intern("state_and_event_indicator"),
                       pmt::from_long(ctx.state_and_event_indicators));
  dict = pmt::dict_add(dict, pmt::intern("data_packet_payload_format"),
                       pmt::from_uint64(ctx.payload_format));

  if (!ctx.raw.empty()) {
    dict = pmt::dict_add(
        dict, pmt::intern("raw"),
        pmt::init_s8vector(ctx.raw.size(), ctx.raw.data()));
  }

  if (ctx.packet_size != ALT_CONTEXT_SIZE) {
    dict = pmt::dict_add(dict, pmt::intern("reference_point"),
                         pmt::from_long(ctx.ref_point));
    dict = pmt::dict_add(dict, pmt::intern("reference_level"),
                         pmt::from_float(ctx.ref_level));
    dict = pmt::dict_add(dict, pmt::intern("rf_gain"),
                         pmt::from_float(ctx.rf_gain));
    dict = pmt::dict_add(dict, pmt::intern("if_gain"),
                         pmt::from_float(ctx.if_gain));
    dict = pmt::dict_add(dict, pmt::intern("timestamp_adjustment"),
                         pmt::from_long(ctx.timestamp_adjustment));
    dict = pmt::dict_add(dict, pmt::intern("timestamp_calibration_time"),
...
difi_context difi::pmt_to_context(pmt::pmt_t dict) {
  difi_context ctx;
  if (!pmt::is_dict(dict)) {
    return ctx;
  }
  ...
  if (pmt::is_integer(p_state)) {
    ctx.state_and_event_indicators =
        static_cast<uint32_t>(pmt::to_long(p_state));
  }
...
pmt::pmt_t difi::make_pkt_n_dict(uint8_t pkt_n,
                                 size_t data_len,
                                 uint32_t full,
                                 uint64_t frac) {
  pmt::pmt_t dict = pmt::make_dict();
  dict = pmt::dict_add(dict, pmt::intern("pck_n"),
                       pmt::from_uint64(static_cast<uint64_t>(pkt_n)));
  dict = pmt::dict_add(dict, pmt::intern("data_len"),
                       pmt::from_uint64(static_cast<uint64_t>(data_len)));
  dict = pmt::dict_add(dict, pmt::intern("full"),
                       pmt::from_long(static_cast<long>(full)));
  dict = pmt::dict_add(dict, pmt::intern("frac"), pmt::from_uint64(frac));
  return dict;
}

// Explicit template instantiations
template void difi::pack_samples<gr_complex>(const gr_complex *,
                                             size_t,
                                             void *,
                                             int,
                                             int,
...
template size_t difi::unpack_samples<gr_complex>(const void *,
                                                 size_t,
                                                 gr_complex *,
                                                 size_t,
                                                 int);
